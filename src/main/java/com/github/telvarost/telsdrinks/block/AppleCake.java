...
public class AppleCake extends TemplateCakeBlock {
    public AppleCake(Identifier identifier, int texture) {
    ...
    public int getTexture(int side, int meta) {
        if (side == 1) {
            return TelsDrinks.APPLE_CAKE_TOP_TEXTURE;
        } else if (side == 0) {
            return this.textureId + 3;
        } else {
            return meta > 0 && side == 4 ? TelsDrinks.APPLE_CAKE_INSIDE_TEXTURE : TelsDrinks.APPLE_CAKE_SIDE_TEXTURE;
        }
    }
    ...
    public int getTexture(int side) {
        if (side == 1) {
            ...
            return side == 0 ? this.textureId + 3 : TelsDrinks.APPLE_CAKE_SIDE_TEXTURE;
        }
    }
